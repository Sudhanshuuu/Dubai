...
const NewProject = ({ createModal, query }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL}/realEstate?type=NewProject`);
                ...
                <tbody >
                    {data.map((data, index) => (
                        ...
                        >
                            ...
                            <td className="px-6 py-2 hidden md:py-4 lg:table-cell">{data.desc.slice(0, 30)} ....</td>
