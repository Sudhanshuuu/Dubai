...
export const Modal = ({ isOpen, onClose, id }) => {
    ...
    useEffect(() => {
        async function getData() {
            await axios.get(`${process.env.REACT_APP_API_URL}/listAProperty/${id}`)
                .then(response => {
    ...
    async function handleSubmit(e) {
        ...
        try {
            const response = await axios.put(`${process.env.REACT_APP_API_URL}/listAProperty/${id}`, formData);
                        ...
                        <div>
                            <form onSubmit={handleSubmit} className='w-[90vw] sm:w-[80vw]' enctype="multipart/form-data">
                                <div className='flex'>
                                    ...
                                    <div className='flex flex-1 flex-col my-[1px] mx-2'>
                                        <label className='my-1 text-sm text-left' htmlFor='name'>Email <span className='text-red-500'>*</span></label>
                                ...
                                <div className='flex'>
                                    ...
                                    <div className='flex flex-1 flex-col my-[1px] mx-2'>
                                        <label className='my-1 text-sm text-left' htmlFor='address'>Address <span className='text-red-500'>*</span></label>
                                        <textarea name='address' value={formData.address} onChange={handleChange} className='border px-2 w-[100%] py-1 h-[15vh] rounded-xl text-sm' />
                                    </div>
                                </div>
                                <div>
                                ...
                                <div className='flex'>
                                    <div className='flex flex-1 flex-col my-[1px] mx-2'>
                                        <label className='my-1 text-sm text-left' htmlFor='time'>Time <span className='text-red-500'>*</span></label>
                                        <input type="text" name='time' value={formData.time} onChange={handleChange} className='border px-2 w-[100%] py-1 rounded-xl text-sm' />
                                    </div>
                                    <div className='flex flex-1 flex-col my-[1px] mx-2'>
                                        <label className='my-1 text-sm text-left' htmlFor='date'>Date <span className='text-red-500'>*</span></label>
                                        <input type="text" name='date' value={formData.date} onChange={handleChange} className='border px-2 w-[100%] py-1 rounded-xl text-sm' />
                                    </div>
                                </div>
